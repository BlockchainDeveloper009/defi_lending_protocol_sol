...
pub fn process_deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
    ...
    // This is important for ensuring the safety and reliability of the code, 
    //  especially in a financial context.
    // The checked_ prefix is a convention in Rust to indicate that the operation will check for errors.
    //if the bank has no deposits, set the initial values to avoid division by zero
...
//here shares are used to avoid burning/minting tokens, but in the future, 
//we can add a minting function to mint new tokens for the user
